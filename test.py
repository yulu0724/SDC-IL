...
parser.add_argument('-mapping_test', help='Print more data', action='store_true')
...
for task_id in range(num_task):
    ...
    if task_id == 0:
        ...
        index_train = random_perm[args.base+(task_id-1)*num_class_per_task:args.base+task_id*num_class_per_task]

    if args.data=='cifar100':
        trainfolder = CIFAR100(root=traindir, train=True, download=True, transform=transform_train,index = index_train)
        testfolder = CIFAR100(root=traindir, train=False, download=True, transform=transform_test,index = index)
    else:
    ...
    train_loader = torch.utils.data.DataLoader(trainfolder, batch_size=128, shuffle=False, drop_last=False)
    test_loader = torch.utils.data.DataLoader(testfolder, batch_size=128, shuffle=False, drop_last=False)
    print('Test %d\t' % task_id)

    model = torch.load(models[task_id])

    train_embeddings_cl, train_labels_cl = extract_features(model, train_loader, print_freq=32, metric=None)
    val_embeddings_cl, val_labels_cl = extract_features(model, test_loader, print_freq=32, metric=None)
    ...
    if task_id > 0 and args.mapping_test:
        ...
        train_embeddings_cl_old, train_labels_cl_old = extract_features(model_old, train_loader, print_freq=32, metric=None)

        MU = np.asarray(class_mean[:args.base+(task_id-1)*num_class_per_task])
        gap = displacement(train_embeddings_cl_old,train_embeddings_cl,MU,args.sigma_test)
        MU += gap   
    ...
    for k in range(task_id+1):
        if k==0:
            ...
            tmp = random_perm[args.base+(k-1)*num_class_per_task:args.base+k*num_class_per_task]
        gt = np.isin(val_labels_cl, tmp)   

        pairwise_distance = euclidean_distances(val_embeddings_cl,np.asarray(class_mean))
