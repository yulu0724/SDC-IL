...
def compute_prototype(model,data_loader):
    ...
    prototype = {'class_mean_old':class_mean,'class_mean':class_mean,'class_std':class_std,'class_label':class_label}
...
def train_fun(args,train_loader,feat_loader, current_task,fisher={},prototype ={}):
    ...
    if args.method == 'Independent' or current_task==0:
        ...
        if args.net == 'resnet32':
            if args.base==50:
                pretrained_dict = torch.load('pretrained_models/Finetuning_0_task_0_200_model_task2_cifar100_seed1993.pkl') 
            ...
            if args.base == 50:
                pretrained_dict = torch.load('pretrained_models/Finetuning_0_task_0_200_model_task2_imagenet_sub_seed1993.pkl')
            pretrained_dict = pretrained_dict.state_dict()
            pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict and 'fc' not in k}
            model_dict.update(pretrained_dict)
            model.load_state_dict(model_dict)
    ...
    if args.method != 'Independent' and current_task>0:
        model = torch.load(os.path.join(log_dir, args.method + '_' + args.exp+ '_task_' + str(current_task-1) + '_%d_model.pkl' % int(args.epochs-1)))
    ...
    model = model.cuda()
    torch.save(model,os.path.join(log_dir, args.method + '_' + args.exp+ '_task_' + str(current_task) +'_pre_model.pkl'))
    ...
    for epoch in range(args.start, args.epochs):
        ...
        for i, data in enumerate(train_loader, 0):
            ...
            if current_task == 0:
                ...
                if args.method == 'Finetuning' or args.method =='Independent' :
                    ...
                    loss_aug = args.tradeoff * torch.sum((embed_feat-embed_feat_old).pow(2))/2.
                    ...
                    for (name,param),(_,param_old) in zip(model.named_parameters(),model_old.named_parameters()): 
                        loss_aug+=args.tradeoff * torch.sum(fisher[name]*(param_old-param).pow(2))/2.
        ...
        if epoch % args.save_step == 0:
            torch.save(model, os.path.join(log_dir, args.method + '_' + args.exp+ '_task_' + str(current_task) + '_%d_model.pkl' % epoch))
    
    if args.method =='EWC' or args.method =='MAS':
        fisher = fisher_matrix_diag(model,criterion,train_loader,number_samples=500) 
        return fisher
...
if __name__ == '__main__':
    ...
    parser.add_argument('-tradeoff', type=float, default=1.0, help="learning rate of new parameters")
    parser.add_argument('-exp', type=str, default='exp1', help="learning rate of new parameters")
    ...
    parser.add_argument("-method",type=str, default='Finetuning',help='Choose FT or SC')

    parser.add_argument('-mapping_mean', default= 'no', type=str, help='mapping')
    ...
    for i in range(num_task):
        ...
        if args.data=='cifar100':
            trainfolder = CIFAR100(root=traindir, train=True, download=True, transform=transform_train,index = class_index)
        else:
            trainfolder = ImageFolder(traindir, transform_train,index = class_index)    

        train_loader = torch.utils.data.DataLoader(
            trainfolder, batch_size=args.BatchSize,
            sampler=RandomIdentitySampler(trainfolder, num_instances=args.num_instances),
        ...
        if args.method == 'EWC' or args.method == 'MAS':
            fisher = train_fun(args,train_loader,feat_loader,i,fisher=fisher)
        else:
